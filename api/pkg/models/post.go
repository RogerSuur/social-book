...
func (repo PostRepository) GetAllByUserId(id int64, offset int64) ([]*FeedPost, error) {
	...
	stmt := `SELECT p.id, p.user_id, u.nickname, p.content, p.created_at, p.image_path, p.privacy_type_id, p.group_id, COUNT(DISTINCT c.id) FROM posts p
	...
	for rows.Next() {
		...
		err := rows.Scan(&post.Id, &post.UserId, &post.UserName, &post.Content, &post.CreatedAt, &post.ImagePath, &post.PrivacyType, &post.GroupId, &post.CommentCount)
		if err != nil {
			return nil, err
		}
...
func (m PostRepository) GetAllFeedPosts(currentUserId int64, offset int64) ([]*FeedPost, error) {
	...
	stmt := `SELECT p.id, p.user_id, u.nickname, p.content, p.created_at, p.image_path, privacy_type_id, p.group_id, COUNT(DISTINCT c.id) FROM posts p 
	LEFT JOIN users u on
	p.user_id = u.id
